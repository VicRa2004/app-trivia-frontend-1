...
const Dashboard = () => {
  ...
  const renderQuizCard = (q: Quiz, isMine: boolean) => (
    <Card key={q.id} className="group hover:-translate-y-1 hover:shadow-xl transition-all duration-300 shadow-sm border-border flex flex-col">
      ...
      <div className="flex flex-col gap-10 w-full animate-in fade-in duration-300 pb-12">
        ...
        <section>
          ...
          {isMyLoading ? (
            ...
            <Card className="flex flex-col items-center justify-center p-12 bg-red-50/50 border-red-100">
               ...
               <p className="text-red-500 mt-2 text-center">No pudimos cargar tus trivias.</p>
            </Card>
          ) : myQuizzes.length === 0 ? (
        ...
        <section>
          <div className="flex flex-col mb-6">
            <h2 className="text-3xl font-extrabold text-text-main tracking-tight">Quizzes Públicos</h2>
            <p className="text-text-muted mt-1 font-medium text-lg">Explora y juega trivias creadas por la comunidad.</p>
          </div>
          ...
          {isPublicLoading ? (
            ...
            <Card className="flex flex-col items-center justify-center p-12 bg-red-50/50 border-red-100">
               <h2 className="text-xl font-bold text-red-700">Parece que hubo un error</h2>
               <p className="text-red-500 mt-2 text-center">No pudimos cargar las trivias públicas.</p>
            </Card>
          ) : publicQuizzes.length === 0 ? (
            <Card className="flex flex-col items-center justify-center p-12 border-dashed border-2 border-border bg-transparent shadow-none">
              <h2 className="text-xl font-bold text-text-main">Aún no hay Quizzes Públicos</h2>
