  ...
  ({ className, variant, size, fullWidth, icon: Icon, isLoading, children, disabled, ...props }, ref) => {
    return (
      <button
        ...
        {...props}
      >
        <span className="absolute inset-0 bg-gradient-to-t from-white/10 to-transparent opacity-0 hover:opacity-100 transition-opacity duration-200" />
        {isLoading && (
        ...
        {!isLoading && Icon && <Icon className="mr-2 h-5 w-5 relative z-10" />}
